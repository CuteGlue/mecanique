...
% noeuds internes
    ...
    fact=Un(1,:);
% schema D.F.
    Tf=2.0;   %t final de la simulation
    nit=floor(Tf/dt)+1;     %no. de pas de temps
% xi sur ¨¤ (0,0)
    Uaxe=zeros(nit+1,1);
    Uaxe(1)=0.0;
    Uexa=zeros(nit+1,1);
...
%%  error
figure(4);
plot(log(C),log(error),'b-*');
legend('error logatithmique');
xlabel('CFL exprim¨¦ logatithmique');
ylabel('error avec solution exact exprim¨¦ log');
title('Error logatithmique en fonction duCFL logatithmique');
pente = (log(error(4))-log(error(1)))/(log(C(4))-log(C(1)))
